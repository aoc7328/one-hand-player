...
半圓操作盤**平時隱藏**，點畫面任一處開啟（4 秒無操作自動收起）。核心是一套
**可擴充的多模式拇指面板**（達文西調色盤式）。

### 拇指半圓控制面板（核心）

第一層 **5 顆可自訂小圓**，沿弧線排列：拇指最順手的**上半部（靠垂直）放常用功能**，
較不順手的下半部放不常用的（如方向鎖）。每顆小圓有自己的互動模式：

| 互動模式 | 行為 |
|---------|------|
| **tap**（點一下） | 觸發功能（方向鎖、快捷開關、縮放…） |
| **drag2D**（按住 2D 滑） | 兩個參數。**音量·亮度** → 上下音量、左右亮度；**跳轉·速度** → 左右跳轉、上下速度 |
| **tapFlick**（播放/暫停） | 點＝播放/暫停；按住往右/左**撥一下**（看方向不看距離）＝前進/後退一幀，可連撥連跳 |
| **submenu**（快捷開關） | 按住約 1 秒（或點一下）展開**第二層**小圓；第二層**往上滑＝開、往下滑＝關**、點＝切換。預設含循環 / 靜音 / 鎖定 |

預設 5 顆（上→下）：**播放/暫停、音量·亮度、跳轉·速度、快捷開關、方向鎖**。

其他半圓手勢：
- 半圓區**雙擊** = 影片放大 / 縮回；放大後半圓**空白拖曳** = 平移可視範圍
- **方向鎖** = 自動 → 鎖橫 → 鎖直（鎖定後不隨手機轉動改變）

- 「更多 / 自訂」收在頂列 **⋯**（自訂半圓功能、左右手、速度、字幕音軌）。
